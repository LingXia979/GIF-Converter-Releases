...
**v0.1.1 公开测试版：支持 Android 10 及以上。** 本仓库提供 Android 安装包、界面介绍、使用说明及第三方组件材料；应用源码仓库继续私有。

## 获取测试包

[下载 Android 安装包](https://github.com/LingXia979/GIF-Converter-Releases/releases/download/v0.1.1/GIF-Converter-v0.1.1-arm64.apk) · [v0.1.1 测试版发布页](https://github.com/LingXia979/GIF-Converter-Releases/releases/tag/v0.1.1)

文件名为 `GIF-Converter-v0.1.1-arm64.apk`。发布页同时提供本次 API 29 构建的 FFmpeg 对应源码包、第三方许可包、安装说明、免责声明、公开签名证书与 SHA-256 校验清单。旧 v0.1.0 发布文件继续保留。

| 项目 | 当前测试包 |
| --- | --- |
| 安装版本 | 0.1.1，版本代码 7 |
| 设备条件 | Android 10+，arm64-v8a（64 位 ARM 系统） |
| APK 大小 | 23,149,852 字节，约 23.15 MB |
| 真机反馈 | 独立兼容测试版在 Android 10 / 16 均反馈正常；公开包在 Android 16 覆盖升级与示例历史预览通过 |
| 发布包检查 | 正式签名、版本、权限、API 29 原生依赖与文件校验通过；详见[测试记录](TESTING.md) |

APK SHA-256：

```text
2d47da731d7c063fda3033e704c918c684b38f7f6c68a298e290a636530bb154
```

**公开 v0.1.0 可直接覆盖更新，无需卸载。** 新版沿用同一包名和正式签名，正常更新保留历史。独立“兼容测试”版可与正式版并存，两边历史分开；更早的同包名调试签名版请先阅读[安装说明](INSTALLATION.md)。重要 GIF、PNG 和 ZIP 建议另存。

已在 BKQ AN80 / Android 16 验证公开 v0.1.0 → v0.1.1 覆盖更新成功，1 条新建示例历史保留且能预览。使用正式签名版后，后续从本仓库下载新版 APK 安装即可更新，不需要每次卸载；详见[后续更新方式](INSTALLATION.md#以后如何更新)。

**更新提示“签名不一致”时，请先保留当前 App。** 早期直接安装的内测版需要原内测签名的专用升级包，维护者已准备此更新方式；请按[安装来源对照表](INSTALLATION.md)选择，避免卸载导致历史丢失。公开 v0.1.0 与 v0.1.1 的正式证书已核对一致。
...
本次 v0.1.1 将最低系统降至 Android 10，保留历史按功能/日期筛选、可拖动的日期范围，以及固定位置的居中选项按钮。沿用粉紫色举牌图标，适配 Android 圆形、圆角和系统主题图标。
...
兼容测试版在 Android 10（型号未提供）和 Android 16（维护者设备 BKQ AN80）已收到正常使用反馈。最终正式签名 APK 已完成包体检查，并在 Android 16 验证公开版之间的覆盖更新、冷启动和已有示例历史预览；其余功能尚未对最终包逐项复测。其他机型、Android 11～15 各版本及真正的 16 KB 页面环境仍待补充验证。相册可能忽略 GIF 循环次数，对透明背景或缩放的显示也可能不同。
