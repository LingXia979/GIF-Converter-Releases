...
**v0.1.2 公开测试版：新增 GIF 拼接旋转与越界裁切。** 支持 Android 10 及以上的 arm64 手机。本仓库提供 Android 安装包、界面介绍、使用说明及第三方组件材料；应用源码仓库继续私有。

## 获取测试包

[下载 Android 安装包](https://github.com/LingXia979/GIF-Converter-Releases/releases/download/v0.1.2/GIF-Converter-v0.1.2-arm64.apk) · [v0.1.2 测试版发布页](https://github.com/LingXia979/GIF-Converter-Releases/releases/tag/v0.1.2)

文件名为 `GIF-Converter-v0.1.2-arm64.apk`。发布页同时提供与 APK 匹配的 FFmpeg 对应源码包、第三方许可包、安装说明、免责声明、公开签名证书与 SHA-256 校验清单。旧 v0.1.0 / v0.1.1 发布文件继续保留。

| 项目 | 当前测试包 |
| --- | --- |
| 安装版本 | 0.1.2，版本代码 9 |
| 设备条件 | Android 10+，arm64-v8a（64 位 ARM 系统） |
| APK 大小 | 23,150,100 字节，约 23.15 MB |
| 真机反馈 | 本版在 Android 16 验收旋转与裁切，9 项原生回归通过，覆盖更新保留原有 8 条历史；Android 10 兼容反馈来自上一版 |
| 发布包检查 | 正式签名、版本、权限、API 29 原生依赖与文件校验通过；详见[测试记录](TESTING.md) |

APK SHA-256：

```text
1326732dfa5fb327a427dd106382b048b23717015e99fc0289779b66f63f6125
```

**公开 v0.1.0 / v0.1.1 可直接覆盖更新，无需卸载。** 新版沿用同一包名和正式签名，正常更新保留历史。独立“兼容测试”版可与正式版并存，两边历史分开；更早的同包名调试签名版请先阅读[安装说明](INSTALLATION.md)。重要 GIF、PNG 和 ZIP 建议另存。

已在 BKQ AN80 / Android 16 验证公开 v0.1.1 → 本版覆盖更新成功，原有 8 条历史保留且旧记录能预览。安装包与已验收的 v0.1.2 build 9 完全一致，已经安装该版本无需重装。以后从本仓库下载新版 APK 安装即可更新；详见[后续更新方式](INSTALLATION.md#以后如何更新)。

**更新提示“签名不一致”时，请先保留当前 App。** 早期直接安装的内测版需要原内测签名的专用升级包；请按[安装来源对照表](INSTALLATION.md)选择，避免卸载导致历史丢失。公开 v0.1.0、v0.1.1 与 v0.1.2 的正式证书已核对一致。

## v0.1.2 新增了什么

- **直接旋转 GIF：** 列表按钮逐张转 90°，四种拼接方式均可使用；无需先到 GIF 编辑处理。
- **画布自由旋转：** 双指同时旋转和缩放，另有角度滑块、左右转 90° 和归正。
- **自动转向排列：** 可选开关，自动排列时尝试转 90°，减少横竖图混排的留白；手动拖动时保留角度。
- **超出画布自动裁切：** 可放大或把素材移出边缘，生成时裁掉超出部分，保留透明/浅灰背景与可选网格。
- **修复放大边缘：** 小 GIF 放大拼接时保持正确的边缘不透明度。

从“GIF 拼接”选择“画布拼接”→“调整画布和位置”即可体验。完整改动与验证范围见[本版发布说明](RELEASE_NOTES.md)。
...
上一版兼容测试在 Android 10（型号未提供）和 Android 16（维护者设备 BKQ AN80）已收到正常使用反馈。本版正式签名 APK 在 Android 16 验证覆盖更新、已有历史、旋转生成和相册保存，用户确认双指操作和结果正常；本次旋转功能未在 Android 10～15 逐项复测。其他机型及真正的 16 KB 页面环境仍待补充验证。相册可能忽略 GIF 循环次数，对透明背景或缩放的显示也可能不同。
